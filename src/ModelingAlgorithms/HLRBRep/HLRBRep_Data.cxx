...
// Linux warning :  assignment to `int' from `double'. Cast has been added.
  ...
  for (Standard_Integer face = 1; face <= myNbFaces; face++) {
	...
	if (!myFEDouble &&
	  ...
	  HLRBRep_EdgeData* ed_ = &(myEData(myFE));
	  myFEGeom = &(ed_->ChangeGeometry());
	  const HLRBRep_Curve& EC = ed_->Geometry();
    ...
    for (myFaceItr1.InitEdge(*fd);
	 ...
	 myFaceItr1.NextEdge()) {
      myFE = myFaceItr1.Edge();
      HLRBRep_EdgeData* ed_ = &(myEData(myFE));
      if (!fd->Simple()) ed_->AutoIntersectionDone(Standard_False);
      HLRAlgo::DecodeMinMax(ed_->MinMax(),
