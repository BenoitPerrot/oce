...
#include <ModelingAlgorithms/TopOpeBRepBuild/TopOpeBRepBuild_kpresu.hxx>
...
//    Standard_Integer iIN = myDataStructure->Shape(*pfIN);
  ...
  if      ( ires == RESNULL ) {
    ...
    for (; itm1.More();itm1.Next()) {
      ...
      const TopoDS_Shape& f2 = lf2.front();
...
/*#ifdef OCCT_DEBUG
      ...
      for (auto s2 : lf2) 
	ChangeMerged(s2,statemergef2).push_back(fac);
...
#endif
    for (const TopoDS_Shape& shecur : loshe1) {
...
#endif
    for (const TopoDS_Shape& shecur : loshe2) {
      myBuildTool.AddSolidShell(newsol,shecur);
    }
  }
...
{  
  ...
  for (const TopoDS_Shape& f1 : lfhsd1) {
...
//      Standard_Boolean isb2 = myKPMAPf1f2.IsBound(f2); // DEB
  ...
  DS.InitSectionEdges();
  for (auto S : les)
    DS.AddSectionEdge(TopoDS::Edge(S));
...
//    fac = TopoDS::Face(Fac1.EmptyCopied());
  ...
  for (const TopoDS_Shape& Fac2 : LF2) {
     ...
     // <lfhsd> : the list of <sol>'s same domain faces, none of the list carries geometric interf
{
  ...
  if ( nfhg != 0 ) {
    for (auto sfhg : lfhg) {
      for (auto x : BDS.ShapeInterferences(sfhg)) {
                                                     ...
                                                     TopTools_ListOfShape& lfhsd) const 
{
  ...
  for (const TopoDS_Shape& fac : lfhsd) {
