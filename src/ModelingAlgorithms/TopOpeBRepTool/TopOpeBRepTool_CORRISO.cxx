...
#include <ModelingAlgorithms/TopOpeBRepTool/TopOpeBRepTool_PURGE.hxx>
...
//	if (supl) xsup = (ivf == i || ivf == 3) ? xi : -1.e7;
  ...
  for (auto sce : ClEds) {
    // cE : 
    const TopoDS_Edge& cE = TopoDS::Edge(sce);
    TopTools_Array1OfShape vcE(1,2); TopOpeBRepTool_TOOL::Vertices(cE,vcE); 
    TopAbs_Orientation ocE = cE.Orientation(); 
					      ...
					      TopTools_DataMapOfOrientedShapeInteger& FyEds, const Standard_Boolean stopatfirst) const 
{
  ...
  for (auto schk : EdsToCheck) {

    const TopoDS_Edge& Echk = TopoDS::Edge(schk);
                                                            ...
                                                            const TopoDS_Edge& E)
{
  ...
  // <myEds> : 
  if (isb) {
    TopTools_ListIteratorOfListOfShape it = begin(myEds);
    while (it != end(myEds)) {
      if (it->IsEqual(E)) {
	it = myEds.erase(it);
	break;
  ...
  for (; exv.More(); exv.Next()){
    ...
    TopTools_ListIteratorOfListOfShape ite = begin(loe);
    while (ite != end(loe)) {
      if (ite->IsEqual(E)) {
	done = Standard_True;
	ite = loe.erase(ite);
