...
TopoDS_Face BRepBuilderAPI_Sewing::WhichFace(const TopoDS_Edge& theEdg, const Standard_Integer index) const
{
  ...
  if (myBoundFaces.Contains(bound)) {
    ...
    for (auto S : myBoundFaces.FindFromKey(bound)) {
      if (i == index)
					       ...
					       const Handle(BRepTools_ReShape)& locReShape)
{
  ...
  if (mySewing) {
    ...
    if (myBoundFaces.Contains(bnd1)) {
      for (auto S : myBoundFaces.FindFromKey(bnd1))
	if (MergedFaces.Add(S))
	  listFaces1.push_back(S);
    }
  }
  else {
					       ...
					       const Standard_Boolean firstCall)
{
  ...
  if (!listFacesFirst.size() || !listFacesLast.size()) return TopoDS_Edge();
  ...
  TopoDS_Face fac2;
  for (auto S2 : (whichSec == 1) ? listFacesLast : listFacesFirst) {
    Handle(Geom2d_Curve) c2d2, c2d21;
    Standard_Real firstOld, lastOld;
    fac2 = TopoDS::Face(S2);
    ...
    for (auto S1 : (whichSec == 1) ? listFacesFirst : listFacesLast) {
      Handle(Geom2d_Curve) c2d1, c2d11;
				       ...
				       const Standard_Integer indRef) const
{
  ...
  for (i = indRef; i <= lengSec; i++) {
    ...
    if (myBoundFaces.Contains(bnd)) {
      face = TopoDS::Face(myBoundFaces.FindFromKey(bnd).front());
                                                 ...
                                                 const Standard_Boolean evalDist)
{
  ...
  if(workfaces.empty()) return;
  TopTools_MapOfShape mapFaces;
  for (auto S : workfaces)
    mapFaces.Add(S);
  ...
  for(Standard_Integer i = 1; i<= seqIndCandidate.Length(); ) {
    ...
    if(!isRemove) {
      ...
      if(myBoundFaces.Contains(bnd2)) {
        ...
        for (auto S : listfaces) {
          if(mapFaces.Contains(S)) {
            TopLoc_Location loc;
            Handle(Geom_Surface) surf = BRep_Tool::Surface(TopoDS::Face(S),loc);
            isMerged = ((IsUClosedSurface(surf,bnd2,loc) ||  IsVClosedSurface(surf,bnd2,loc)) && 
              IsMergedClosed(TopoDS::Edge(sequenceSec.Value(index)),TopoDS::Edge(workedge),TopoDS::Face(S)));
          }
	  if (!isMerged)
	    break;
        }
                                                       ...
                                                       TColStd_SequenceOfInteger& seqOrientations)
{
  ...
  { //szv: Use brackets to destroy local variables
    ...
    if (mySectionBound.IsBound(bnd)) bnd = mySectionBound(bnd);
    if (myBoundFaces.Contains(bnd)) {
      for (auto S : myBoundFaces.FindFromKey(bnd))
	Faces1.Add(S);
    }
  }
  ...
  if(nbCandidates > 0)
  {
    ...
    if (myBoundFaces.Contains(bnd)) {
      ...
      for (auto S : myBoundFaces.FindFromKey(bnd)) {
        const TopoDS_Face& Face2 = TopoDS::Face(S);
...
void BRepBuilderAPI_Sewing::Dump() const
{
  ...
  for (i = 1; i <= NbBounds; i++) {
    ...
    if (myBoundSections.IsBound(bound)) NbSections += myBoundSections(bound).size();
    else NbSections++;
...
void BRepBuilderAPI_Sewing::FaceAnalysis(const Handle(Message_ProgressIndicator)& thePI)
{
  ...
  for (i = 1; i <= myOldShapes.Extent() && aPS.More(); i++, aPS.Next()) {
	...
	for (; eiter.More(); eiter.Next()) {
	  ...
	  if (!isSmall) {
		...
		GluedVertices(nv2).push_back(v1);
  ...
  for (; miter.More(); miter.Next()) {
    ...
    for (auto S : vlist) {
      coord += BRep_Tool::Pnt(TopoDS::Vertex(S)).XYZ();
      nbPoints++;
    }
    if (nbPoints) {
      ...
      for (auto S : vlist) {
	Standard_Real vtol = BRep_Tool::Tolerance(TopoDS::Vertex(S));
	if (mtol < vtol) mtol = vtol;
	vtol = vp.Distance(BRep_Tool::Pnt(TopoDS::Vertex(S)));
...
void BRepBuilderAPI_Sewing::FindFreeBoundaries()
{
    ...
    for (; fExp.More(); fExp.Next()) {
      ...
      for (TopoDS_Iterator aIw(face); aIw.More(); aIw.Next()) {
        ...
        for (TopoDS_Iterator aIIe(aIw.Value()); aIIe.More(); aIIe.Next()) {
          ...
          if (EdgeFaces.Contains(edge)) {
            EdgeFaces.ChangeFromKey(edge).push_back(face);
  ...
  for (i = 1; i <= nbShapes; i++) {
    ...
    Standard_Integer nbFaces = listFaces.size();
    TopoDS_Shape edge    = EdgeFaces.FindKey(i);
    if(edge.Orientation() == TopAbs_INTERNAL)
          continue;
    Standard_Boolean isSeam = Standard_False;
    if (nbFaces == 1) {
      TopoDS_Face face = TopoDS::Face(listFaces.front());
	...
	//isSeam = (IsUClosedSurface(surf) || IsVClosedSurface(surf));
        //if(!isSeam) {
          ...
          Handle(Geom2d_Curve) c2dold = 
            BRep_Tool::CurveOnSurface(TopoDS::Edge(edge),TopoDS::Face(listFaces.front()),first2d,last2d);

          Handle(Geom2d_Curve) c2d;
          BRep_Builder B;
          B.UpdateEdge(TopoDS::Edge(anewEdge),c2d,c2d,TopoDS::Face(listFaces.front()),0);
          B.UpdateEdge(TopoDS::Edge(anewEdge),c2dold,TopoDS::Face(listFaces.front()),0);
          
          Standard_Real aFirst, aLast;
          BRep_Tool::Range(TopoDS::Edge(edge),aFirst, aLast);
          aB.Range(TopoDS::Edge(anewEdge),aFirst, aLast);
          aB.Range(TopoDS::Edge(anewEdge),TopoDS::Face(listFaces.front()),first2d,last2d);
    ...
    if (isBound || isBoundFloat) {
     ...
     // if (nbVtx != 2) continue;
      ...
      listFacesCopy.insert(end(listFacesCopy), begin(listFaces), end(listFaces));
				       ...
				       TopTools_DataMapOfShapeListOfShape& aNodeEdges)
{
  ...
  for (i = 1; i <= nbNearest; i++) {
	...
	if (!newnode1.IsSame(newnode2)) {
	  ...
	  for (auto node2 : NewNodeOldNodes(newnode2)) {
	    lnode1.push_back(node2);
	...
	NewNodeOldNodes(newnode1).push_back(oldnode2);
      }
    }
    else {
      if (OldNodeNewNode.IsBound(oldnode2)) {
	...
	NewNodeOldNodes(newnode2).push_back(oldnode1);
      }
      else {
	...
	lnodes.push_back(oldnode1);
	lnodes.push_back(oldnode2);
  ...
  for (; iter1.More(); iter1.Next()) {
    ...
    for (auto oldnode : iter1.Value()) {
      // Iterate on node vertices
      for (auto S : NodeVertices.FindFromKey(oldnode)) {
	TopoDS_Vertex vertex = TopoDS::Vertex(S);
	// Change node for vertex
	aVertexNode.ChangeFromKey(vertex) = newnode;
	// Accumulate coordinates
	theCoordinates += BRep_Tool::Pnt(vertex).XYZ();
	lvert.push_back(vertex);
      }
      // Iterate on node edges
      for (auto edge : aNodeEdges(oldnode)) {
	if (!medge.Contains(edge)) { medge.Add(edge); ledge.push_back(edge); }
      }
    ...
    gp_Pnt center(theCoordinates / lvert.size());
                                     ...
                                     const Handle(Message_ProgressIndicator)& theProgress)
{
  ...
  for (i = 1; i <= nbVertices; i++) {
    ...
    if (NodeVertices.Contains(node)) {
      NodeVertices.ChangeFromKey(node).push_back(vertex);
    }
    else {
      TopTools_ListOfShape vlist;
  ...
  for (i = 1; i <= nbNodes && aPS.More(); i++, aPS.Next()) {
	...
	// Explore edges of the second node
	  ...
	  if (lfac2.size()) {
	    for (auto S : lfac2) {
	      if (Status || isSameEdge)
		break;
	      // Check merging conditions for the same face
	      if (Faces1.Contains(S)) {
		Standard_Integer stat = IsMergedVertices(S,e1,e2,v1,v2);
	...
	if (Status || isSameEdge)
      ...
      if (Status) continue;
      if (isSameEdge) listNodesSameEdge.push_back(node2);
    ...
    if (SeqNodes.Length()) {
      ...
      if (listNodesSameEdge.size()) {
	for (auto S : listNodesSameEdge) {
	  const TopoDS_Vertex& n2 = TopoDS::Vertex(S);
...
void BRepBuilderAPI_Sewing::VerticesAssembling(const Handle(Message_ProgressIndicator)& thePI)
{
  ...
  if (nbVert || nbVertFree) {
	...
	if (myNodeSections.IsBound(node))
	  myNodeSections(node).push_back(bound);
	else {
	  TopTools_ListOfShape lbnd;
	  lbnd.push_back(bound);
                                    ...
                                    const Handle(Message_ProgressIndicator)& thePI)
{
  ...
  for (Standard_Integer i = 1; i <= myBoundFaces.Extent() && aPS.More(); i++, aPS.Next()) {
    ...
    if (hasCuttingSections) {
      ...
      for (auto section : myBoundSections(bound)) {
        ...
        while (ii <= seqMergedWithBound.Length()) {
          ...
          if (!isRejected) {
            if (myBoundSections.IsBound(iedge)) {
              ...
              for (auto sec : myBoundSections(iedge)) {
    ...
    if (isMerged && isMergedSplit && !isPrevSplit) {
      ...
      for (itm.Initialize(MergedWithBound); itm.More(); itm.Next()) {
        ...
        if (myBoundSections.IsBound(edge)) {
          ...
          for (auto sec : myBoundSections(edge)) {
                                                           ...
                                                           TColStd_SequenceOfInteger& SeqMergedOri)
{
  ...
  if (myCuttingNode.IsBound(nno1)) {
    for (auto v1 : myCuttingNode(nno1)) {
      mapVert1.Add(v1);
      if (!isNode1 && myCuttingNode.IsBound(v1)) {
        for (auto vn : myCuttingNode(v1)) {
          mapVert1.Add(vn);
        }
      }
    }
  ...
  if (myCuttingNode.IsBound(nno2)) {
    for (auto v1 : myCuttingNode(nno2)) {
      mapVert2.Add(v1);
  ...
  for (TopTools_MapIteratorOfMapOfShape imv1(mapVert1); imv1.More(); imv1.Next()) {
    ...
    for (auto sec : myNodeSections(node1)) {
      ...
      if ((mapVert1.Contains(vs1n) && mapVert2.Contains(vs2n)) ||
        ...
        if (mapEdges.Add(sec)) {
          ...
          {
            for (auto section : myBoundSections(sec)) {
...
void BRepBuilderAPI_Sewing::Cutting(const Handle(Message_ProgressIndicator)& thePI)
{
  ...
  for (i = 1; i <= nbBounds && aPS.More(); i++, aPS.Next()) {
    ...
    if (!myBoundFaces(i).size()) continue;
    // Obtain bound curve
      ...
      for (auto section : listSections) {
        ...
        for (TopoDS_Iterator itv(section); itv.More(); itv.Next()) {
          ...
          if (myNodeSections.IsBound(vertex))
            myNodeSections.ChangeFind(vertex).push_back(section);
          else {
            TopTools_ListOfShape lsec;
            lsec.push_back(section);
...
void BRepBuilderAPI_Sewing::GetFreeWires(TopTools_MapOfShape& MapFreeEdges, TopTools_SequenceOfShape& seqWires)
{
  ...
  for (; itMap.More(); itMap.Next()) {
    ...
    for (TopoDS_Iterator Iv(edge,Standard_False); Iv.More(); Iv.Next()) {
      TopoDS_Vertex V1 = TopoDS::Vertex(Iv.Value());
      if (VertEdge.IsBound(V1))
        VertEdge.ChangeFind(V1).push_back(edge);
      else {
  ...
  for (Standard_Integer i = 1; i <= myBoundFaces.Extent() && aPS.More(); i++, aPS.Next()) {
    ...
    if (listFaces.size() == 1) {
      if (myBoundSections.IsBound(bound)) {
        for (auto S : myBoundSections(bound)) {
          if (!myMergedEdges.Contains(S)) { //myReShape->IsRecorded(liter.Value())) {
            TopoDS_Shape edge = myReShape->Apply(S);
            if (!MapFreeEdges.Contains(edge)) {
              TopoDS_Shape face = listFaces.front();
                                               ...
                                               TColStd_SequenceOfReal& seqPara)
{
  ...
  for (; mit.More(); mit.Next()) {
    ...
    if (myCuttingNode.IsBound(node)) {
      myCuttingNode.ChangeFind(node).push_back(cnode);
    }
    else {
      TopTools_ListOfShape ls;
      ls.push_back(cnode);
                                           ...
                                           TopTools_ListOfShape& listEdge)
{
  ...
  for (auto F : listFaces) {
    ...
    const TopoDS_Face& fac = TopoDS::Face(F);
