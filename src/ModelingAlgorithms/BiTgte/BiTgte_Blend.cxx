...
//      const TopoDS_Edge& IE   = TopoDS::Edge(OF.Generated(OriE));
	...
	if (WarningSurBordLibre) {
	  ...
	  if (L.size() == 1) break; // Nothing is done.
	}
	Map.Add(exp.Current());
	break;
      }
...
//      const TopoDS_Edge& IE = TopoDS::Edge(OF.Generated(exp.Current()));
      if ( E.IsEqual(IE)) {
	for (auto ancS : Analyse.Ancestors(exp.Current())) {
	  Map.Add(ancS);
	}
	break;
      }
       ...
       Standard_Real                    Tol)
{
  ...
  for (auto TangS : TangE) {
    if (Find) break;
    const TopoDS_Edge& ET = TopoDS::Edge(TangS);
    if ( MapSF.IsBound(ET)) {
...
//      E1 = TopoDS::Edge(MapSF(ET).Generated(V1));
	...
	CET = Handle(Geom_TrimmedCurve)::DownCast(CET)->BasisCurve();
      ...
      for (auto TangS2 : Tang2) {
	const TopoDS_Edge& ET2 = TopoDS::Edge(TangS2);
...
//	  MapOnV.Add(TopoDS::Edge(MapSF(ET2).Generated(U1)));
  ...
  if (!Find) {
    ...
    if (CenterAnalyse.HasAncestor(V1)) {
      ...
      for (auto S : TangE) {
	if ( MapSF.IsBound(S)) {
	  MapOnV.Add(MapSF(S).Generated(V1));
	}
      }
    }
  }
...
void BiTgte_Blend::ComputeCenters() 
{
  ...
  while ( JenRajoute) {
	...
	for (auto st : Let) {
	  const TopoDS_Edge& Cur = TopoDS::Edge(st);
...
//	    const TopoDS_Edge& OTE = TopoDS::Edge(OF1.Generated(Cur));
	...
	if (!L.empty() && L.front().Type() == OT) {
	  TopoDS_Shape aLocalShape = myMapSF(Anc.front()).Generated(E);
	  TopoDS_Edge EOn1 = TopoDS::Edge(aLocalShape);
	  aLocalShape = myMapSF(Anc.back()) .Generated(E);
...
#warning find
	  ...
	  for (auto S : TangE) {
	    if (Find) break;
	    if ( myMapSF.IsBound(S)) {
	      TopoDS_Shape aLocalShape = myMapSF(S).Generated(V1f);
...
#warning find
	  ...
	  for (auto S : TangE) {
	    if (Find) break;
	    if ( myMapSF.IsBound(S)) {
	      TopoDS_Shape aLocalShape = myMapSF(S).Generated(V1l);
...
//	    TopoDS::Edge(myMapSF(CurF).Generated(CurE));
	  ...
	  if (   !myFaces    .Contains(Lanc.front()) 
	      || !myFaces    .Contains(Lanc.back ())
	      ||  myStopFaces.Contains(Lanc.front()) 
	      ||  myStopFaces.Contains(Lanc.back ())) {
	    TopoDS_Shape aLocalShape = myMapSF(CurF).Generated(CurE);
	    const TopoDS_Edge& CurOE = TopoDS::Edge(aLocalShape);
//	    const TopoDS_Edge& CurOE = 
//	      TopoDS::Edge(myMapSF(CurF).Generated(CurE));
  ...
  for (const TopoDS_Shape& CurLOF : LOF) {
    ...
    for (const TopoDS_Shape& CurLIM : Lim) {
...
void BiTgte_Blend::ComputeSurfaces()
{
  ...
  for (i = 1; i <= myEdges.Extent(); i++) {
	...
	CurL.push_back(CurE);               // I'm on intersection tube-tube
    ...
    // ---------------------------------------------------------------
    for (auto CurCutS : CurL) {
      const TopoDS_Edge& CurCutE = TopoDS::Edge(CurCutS);
	...
	myCreated(CurCutE)(Or1).push_back(E1);

	myCreated(CurCutE).Bind(Or2,Empty);
	myCreated(CurCutE)(Or2).push_back(E2);
      }
...
//			 TopoDS::Edge(Or1),myTol);
	  ...
	  L1.push_back(V1); L1.push_back(V2);
	}
...
//			 TopoDS::Edge(Or2),myTol);
  ...
  for ( Standard_Integer i = 1; i <= Map.Extent(); i++) {
    ...
    for (auto S : Map(i)) {
      ...
      if ( Reverse) 
	LOE.push_back(myMapSF(S).Generated(V).Reversed());
      else 
	LOE.push_back(myMapSF(S).Generated(V));
    }
...
//      const TopoDS_Edge& CurOE = TopoDS::Edge(Offset.Generated(CurE));
	...
	TopoDS::Edge(myImageOffset.Image(CurOE).front());
      if (ImE.IsSame(CurOE)) {
	myCreated.Bind(CurOE,EmptyMap);      
	myCreated(CurOE).Bind(CurF,Empty);
	myCreated(CurOE)(CurF).push_back(CurE);
      }
    }
  }
  ...
  for ( ;exp.More(); exp.Next()) {
	...
	continue;
      ...
      for (auto DeboucS : Lim) {
	// DeboucFace = offset Face unwinded in "Debouc".
	const TopoDS_Face& DeboucFace = TopoDS::Face(DeboucS);
	...
	for ( ; exp2.More(); exp2.Next()) {
	  ...
	  if (myCreated.IsBound(E)) {
	    if (myCreated(E).IsBound(CurF)) {
	      const TopoDS_Edge& OE = TopoDS::Edge(myCreated(E)(CurF).front());
		...
		myCreated(V1)(CurF).push_back(OV1);
	      }
	      if ( !myCreated(V2).IsBound(CurF))  {
		myCreated(V2).Bind(CurF,Empty);
		myCreated(V2)(CurF).push_back(OV2);
	      }
	    }
	  }
	}
...
//					  Face,f,l);
		...
		B.UpdateEdge(OE,C2d_1,C2d,NewF,BRep_Tool::Tolerance(E));
	      ...
	      if ( myCreated.IsBound(E)) {
		if ( myCreated(E).IsBound(CurF)) {
		  OE = TopoDS::Edge(myCreated(E)(CurF).front());
		}
	      }
	      else {
		B.MakeEdge(OE);
		...
		if ( myCreated.IsBound(V1) && myCreated(V1).IsBound(CurF)) {
		  OV1 = TopoDS::Vertex(myCreated(V1)(CurF).front());
		}
		else {
		  B.MakeVertex(OV1);
		  gp_Pnt2d P2d = 
		...
		if ( myCreated.IsBound(V2) && myCreated(V2).IsBound(CurF)) {
		  OV2 = TopoDS::Vertex(myCreated(V2)(CurF).front());
		}
		else {
		  B.MakeVertex(OV2);
		  gp_Pnt2d P2d = 
       ...
       BRepOffset_Inter3d&       Inter) 
{
  ...
  for ( ; It.More(); It.Next()) {
    ...
    if (myAsDes->HasCommonDescendant(Face,F2,LInt)) {
      for (auto CurS : LInt) {
	const TopoDS_Edge& CurE = TopoDS::Edge(CurS);
	TopoDS_Vertex V1,V2;
	TopoDS_Edge   E1,E2;
