 ...
 const TopoDS_Shape& E)
{
  ...
  for(;II.More();II.Next()) {
	...
	Result.push_back(myHDS->Shape(goodIndex));
      else 
	if (goodKind == TopOpeBRepDS_POINT)
	  Result.push_back(myHB->NewVertex(goodIndex));
    }
  }
...
#warning find
  ...
  Empty.Nullify();
  Suppress(C, Empty);
  LLS = myListOfCompoundOfEdgeConnected.erase(LLS);
}
