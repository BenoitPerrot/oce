...
{ 
  ...
  if (Inter.IsDone()) {
	...
	O2 = TopAbs::Reverse(O2);
      ...
      L1.push_back (E.Oriented(O1));
      L2.push_back (E.Oriented(O2));
#ifdef DRAW
      if (AffichInter) {
        char name[256];
			      ...
			      const Standard_Boolean IsRefEdgeDefined)
{
  ...
  for (i = 0; i < aNb; i++) {
	...
	Standard_Real aTolEdge = BRep_Tool::Tolerance(anEdge);
        ...
        L1.push_back (anEdge.Oriented(O1));
        L2.push_back (anEdge.Oriented(O2));
 ...
 const Standard_Real         TolConf)
{
  ...
  for (auto S : Edges) {
	...
	O2 = TopAbs::Reverse(O2);
      ...
      LInt1.push_back (CurE.Oriented(O1));
      LInt2.push_back (CurE.Oriented(O2));
#ifdef DRAW
      if (AffichInter) {
        char name[256];
        sprintf(name,"EI_%d",NbNewEdges++);	
...
//      U = BRep_Tool::Parameter(CV,TopoDS::Edge(E1.Oriented(TopAbs_FORWARD)));
    ...
    LV.clear();LV.push_back(VF); LV.push_back(VL);
  }
  ...
  for (exp.Init(F.Oriented(TopAbs_FORWARD),TopAbs_WIRE); 
	...
	if (Build.IsBound(NEOnV1) && 
	  ...
	  if (E.IsSame(NEOnV1)) 
		...
		V = TopoDS::Vertex(LV.front());
	      }
	      else {
		V = TopoDS::Vertex(LV.back());
	      }
	    }
	    else {
