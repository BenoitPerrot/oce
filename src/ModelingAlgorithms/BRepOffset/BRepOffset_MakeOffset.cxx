			 ...
			 TopTools_DataMapOfShapeShape& MapEF)
{
  ...
  while (!Edges.empty())
    {
      TopoDS_Edge StartEdge = TopoDS::Edge(Edges.front());
      Edges.pop_front();
      ...
      aContour.push_back(StartEdge);
      while (!CurVertex.IsSame(StartVertex))
	for (TopTools_ListIteratorOfListOfShape itl = begin(Edges); itl != end(Edges); ++itl)
	  {
#warning remove_if?
...
static void MakeList (TopTools_ListOfShape&             OffsetFaces,
		      ...
		      const TopTools_IndexedMapOfShape& myFaces)
{
  for (const TopoDS_Shape& Root : myInitOffsetFace.Roots()) {
    if (!myFaces.Contains(Root))
      OffsetFaces.push_back(myInitOffsetFace.Image(Root).front());
  }
}
...
void BRepOffset_MakeOffset::BuildOffsetByInter()
{
  ...
  for (auto S : LF) {
    ...
    for (auto SCur : Let) {
      const TopoDS_Edge& Cur = TopoDS::Edge(SCur);
...
//  Modified by skv - Mon Jan 12 11:50:03 2004 OCC4455 End
  ...
  for (Exp.Init(myShape,TopAbs_FACE) ; Exp.More(); Exp.Next()) {
	...
	if (Build.IsBound(EI)) {
	  ...
	  if (NE.ShapeType() == TopAbs_EDGE) {
		...
		if (!AsDes2d->Descendant(NEC).empty()) {
  ...
  for (auto NES : LFE) {
    const TopoDS_Face& NEF = TopoDS::Face(NES);
  ...
  for (Exp.Init(myShape,TopAbs_FACE) ; Exp.More(); Exp.Next()) {
	...
	for (const TopoDS_Shape& OFE : LOFE) {
	...
	for (auto S : LE) {
	  ...
	  if (IMOE.HasImage(OE)) {
	    ...
	    for (auto OS : LOE) {
	      TopoDS_Shape aLocalShape = OS.Oriented(OE.Orientation());
  ...
  for (Exp.Init(myShape,TopAbs_FACE) ; Exp.More(); Exp.Next()) {
	...
	if (Build.IsBound(anEdgeRef)) {
	  ...
	  if (aNewShape.ShapeType() == TopAbs_EDGE) {
		...
		for (auto S : IMOE.Image(aResEdge))
		  aListNewEdge.push_back(S);
	      } else
		aListNewEdge.push_back(aResEdge);
	    }
...
//  Modified by skv - Tue Mar 15 16:20:43 2005
  ...
  for (i = 1; i <= myFaces.Extent(); i++) {
    ...
    for (auto S : AsDes->Descendant(Cork)) {
      const TopoDS_Edge& OE = TopoDS::Edge(S);
      if (IMOE.HasImage(OE)) {
	const TopTools_ListOfShape& LOE = IMOE.Image(OE);
	for (auto COS : LOE) {
	  const TopoDS_Edge& COE = TopoDS::Edge(COS);
...
//	const TopoDS_Edge& OTE = TopoDS::Edge(OF.Generated(Cur));
  ...
  for (Exp.Init(myShape,TopAbs_EDGE); Exp.More(); Exp.Next()) {
	...
	if (!L.empty() && L.front().Type() == OT) {
	  Standard_Real CurOffset = myOffset;
	  if ( myFaceOffset.IsBound(Anc.front()))
	    CurOffset = myFaceOffset(Anc.front());
	  TopoDS_Shape aLocalShape = MapSF(Anc.front()).Generated(E);
	  TopoDS_Edge EOn1 = TopoDS::Edge(aLocalShape);
	  aLocalShape = MapSF(Anc.back()).Generated(E);
...
#warning find
	  ...
	  for (auto S : TangE) {
	    if (Find) break;
	    if ( MapSF.IsBound(S)) {
	      TopoDS_Shape aLocalShape = MapSF(S).Generated(V1f);
...
#warning find
	  ...
	  for (auto S : TangE) {
	    if (Find) break;
	    if ( MapSF.IsBound(S)) {
	      TopoDS_Shape aLocalShape = MapSF(S).Generated(V1l);
...
///	TopoDS_Edge EOn1 = TopoDS::Edge(MapSF(Anc.First()).Generated(E));
  ...
  for (Exp.Init(myShape,TopAbs_VERTEX); Exp.More(); Exp.Next()) {
    ...
    if (Done.Add(V)) {
      ...
      if (LE.size() >= 3 && LE.size() == LA.size()) {
	TopTools_ListOfShape LOE;
	//--------------------------------------------------------
	// Return connected edges on tubes.
	//--------------------------------------------------------
	for (auto S : LE) {
	  LOE.push_back(MapSF(S).Generated(V).Reversed());
	}
	//----------------------
	// construction sphere.
	//-----------------------
	const TopTools_ListOfShape& LLA = myAnalyse.Ancestors(LA.front());
	const TopoDS_Shape& FF = LLA.front();
	Standard_Real CurOffset = myOffset;
	...
	for (auto S : LE) {
	  if (First) {
	    ...
	    myInitOffsetEdge.Bind(V,MapSF(S).Generated(V));
	    First = Standard_False;
	  }
	  else {
	    myInitOffsetEdge.Add(V,MapSF(S).Generated(V));
	  }
	} 
      }
    }
  ...
  */
  ...
  for (j = 1; j <= myFaces.Extent(); j++) {
    ...
    for (exp.Init(CF.Oriented(TopAbs_FORWARD),TopAbs_EDGE); 
	 exp.More(); exp.Next()) {
      const TopoDS_Edge& E = TopoDS::Edge(exp.Current()); 
      if (myAnalyse.HasAncestor(E)) {
	for (auto AS : myAnalyse.Ancestors(E)) {
	  const BRepOffset_Offset& OF = MapSF(AS);
	  FacesToBuild.Add(AS);
	...
	for (Standard_Integer i = 0; i < 2; i++) {
	  for (auto AS : myAnalyse.Ancestors(V[i])) {
	    const TopoDS_Edge& EV = TopoDS::Edge(AS);
	    if (MapSF.IsBound(EV)) {
  ...
  for (itc.Initialize(Created); itc.More(); itc.Next()) {
    ...
    if (myInitOffsetEdge.IsImage(OE)) {
      ...
      Or = myInitOffsetEdge.Image(E).front().Orientation();
      if (NE.Orientation() == TopAbs_REVERSED) 
	NE.Orientation(TopAbs::Reverse(Or));
      else
	NE.Orientation(Or);
...
void BRepOffset_MakeOffset::CorrectConicalFaces()
{
  ...
  for (; Cone.More(); Cone.Next() ) {
    ...
    for (auto S : Faces) {
      TopoDS_Face aFace = TopoDS::Face(S); //TopoDS::Face(Faces.First());
    ...
    for (TopTools_ListIteratorOfListOfShape itl = begin(EdgesOfWire); itl != end(EdgesOfWire); ++itl)
    {
      FirstEdge = TopoDS::Edge(*itl);
      Standard_Real f, l;
      ...
      {
        itl = EdgesOfWire.erase(itl);
        break;
      }
    }
    NewEdges.push_back(FirstEdge);
    TopoDS_Vertex Vf1, CurVertex;
    TopExp::Vertices(FirstEdge, Vf1, CurVertex);
    {
      TopTools_ListIteratorOfListOfShape itl = begin(EdgesOfWire);
      while (itl != end(EdgesOfWire)) {
	const TopoDS_Edge& anEdge = TopoDS::Edge(*itl);
	TopoDS_Vertex V1, V2;
	TopExp::Vertices(anEdge, V1, V2);
	if (V1.IsSame(CurVertex) || V2.IsSame(CurVertex)) {
	  NewEdges.push_back(anEdge);
	  CurVertex = (V1.IsSame(CurVertex))? V2 : V1;
	  itl = EdgesOfWire.erase(itl);
	}
	else
    ...
    if (theSubstitutor.IsCopied(NewSphericalFace))
    {
      ...
      NewSphericalFace = TopoDS::Face(listSh.front());
    }
...
void BRepOffset_MakeOffset::MakeFaces(TopTools_IndexedMapOfShape& /*Modif*/)
{
  ...
  // Loop on all faces //.
  //----------------------------------
  for (auto Root : myInitOffsetFace.Roots()) {
    TopoDS_Face F = TopoDS::Face(myInitOffsetFace.Image(Root).front());
    LOF.push_back(F);
  }
...
void BRepOffset_MakeOffset::MakeMissingWalls ()
{
  ...
  for (; iter.More(); iter.Next())
      ...
      for (auto S : aContour)
	{
	  ...
	  myImageOffset.LastImage( LOE.back(), LOE2 );
	  TopoDS_Edge OE = TopoDS::Edge( LOE2.back() );
	  ////////////////////////////////////////////////////////////////////////
	  ...
	  myWalls.push_back(NewFace);
	  if (ArcOnV2)
	    {
	      TopoDS_Edge anArc = TopoDS::Edge(myInitOffsetEdge.Image(V2).front());
...
#endif
  BRepTools_Quilt Glue;
  for (auto Root : myImageOffset.Roots()) {
    TopTools_ListOfShape Image;
    myImageOffset.LastImage(Root,Image);
    for (auto S : Image) {
      Glue.Add(S);
    }
  }
...
void BRepOffset_MakeOffset::EncodeRegularity ()
{
  ...
  for ( ; exp.More(); exp.Next()) {
    ...
    const TopoDS_Face& F1 = TopoDS::Face(LofOF.front());
    const TopoDS_Face& F2 = TopoDS::Face(LofOF.back() );
    ...
    //  the initial shape, they will be tangent in the offset shape
      ...
      BRepOffset_Tool::HasCommonShapes(TopoDS::Face(Root1),
				       ...
				       LE,LV);
      if ( LE.size() == 1) { 
	const TopoDS_Edge& Ed = TopoDS::Edge(LE.front());
