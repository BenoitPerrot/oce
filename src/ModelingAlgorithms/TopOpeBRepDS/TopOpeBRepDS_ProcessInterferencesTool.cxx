...
#warning awfull code duplication!
...
{
  TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(L1));
  while (it1 != end(L1)) {
...
(TopOpeBRepDS_ListOfInterference& L1,const TopOpeBRepDS_Kind GK,TopOpeBRepDS_ListOfInterference& L2)
{
  TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(L1));
  while (it1 != end(L1)) {
    TopOpeBRepDS_Kind gki = (*it1)->GeometryType();
    if (gki == GK) {
      L2.push_back(*it1);
      it1 = L1.erase(it1);
    }
...
(TopOpeBRepDS_ListOfInterference& L1,const TopOpeBRepDS_Kind SK,TopOpeBRepDS_ListOfInterference& L2)
{
    TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(L1));
  while (it1 != end(L1)) {
    TopOpeBRepDS_Kind ski = (*it1)->SupportType();
    if (ski == SK) {
      L2.push_back(*it1);
      it1 = L1.erase(it1);
    }
...
(TopOpeBRepDS_ListOfInterference& L1,const Standard_Integer GI,TopOpeBRepDS_ListOfInterference& L2)
{
  ...
  TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(L1));
  while (it1 != end(L1)) {
    Standard_Integer gi = (*it1)->Geometry();
    if (gi == GI) {
      L2.push_back(*it1);
      it1 = L1.erase(it1);
    }
...
(TopOpeBRepDS_ListOfInterference& L1,const Standard_Integer SI,TopOpeBRepDS_ListOfInterference& L2)
{
  ...
  TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(L1));
  while (it1 != end(L1)) {
    Standard_Integer si = (*it1)->Support();
    if (si == SI) {
      L2.push_back(*it1);
      it1 = L1.erase(it1);
    }
...
//------------------------------------------------------
{
  ...
  TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(lFE));
  while (it1 != end(lFE)) {
...
//      if (sdmFT) {it2.Next(); continue;}      
      if (sdmFT) {lFEresi.push_back(I2); it2 = lFE.erase(it2); continue;}  
	...
	if (sdm) {
      ...
      if (sameSorsharedEbyTRASHA) {l3dFE.push_back(I2); it2 = lFE.erase(it2); complex3d=Standard_True;}
      else                        ++it2;
    }

    if (complex3d) {l3dFE.push_back(I1); it1 = lFE.erase(it1);}
    else           ++it1;
  } // it1(lFE);
  ...
  n3d = l3dFE.size();
  Standard_Integer nresi = lFEresi.size();
  if (nresi == 0) return n3d;

  if      (n3d != 0) {
    TopOpeBRepDS_ListIteratorOfListOfInterference it(begin(lFEresi));
    while (it != end(lFEresi)){
      ...
      if (nfound == 0) it = lFE.erase(it);
      else             ++it;
    }
  }
  else lFE.insert(end(lFE), begin(lFEresi), end(lFEresi));
  return n3d;
}

static Standard_Boolean FUN_find3dISEsameISF(const Handle(TopOpeBRepDS_Interference)& I1, TopOpeBRepDS_ListIteratorOfListOfInterference& it2, const TopOpeBRepDS_ListIteratorOfListOfInterference& end)
// I1 = (T0,G0,S)
...
// I2 = (T0,G0,S')     
{
  ...
  while (it2 != end){
    Handle(TopOpeBRepDS_Interference)& I2 = *it2;
...
//-------------------------------------------------------------
{
  ...
  // <lFold>    -> {I3d'}+ <lFnew>;    {I3d'}=<l3dF>
    ...
    TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(l3dFE));
  while (it1 != end(l3dFE)) {
    const Handle(TopOpeBRepDS_Interference)& I1 = *it1;
//    TopAbs_Orientation O1 = I1->Transition().Orientation(TopAbs_IN);
//    TopOpeBRepDS_Kind GT1,ST1; Standard_Integer G1,S1; FDS_data(I1,GT1,G1,ST1,S1);
...
//    else       {lFE.Append(I1); l3dFE.Remove(it1);}
    ...
    // => l3dFE += IFE, l3dF += iFF
    TopOpeBRepDS_ListIteratorOfListOfInterference it2(begin(lF));
    Standard_Boolean found = FUN_find3dISEsameISF(I1,it2,end(lF));
    ...
    if (found) {
      ...
      // => l3dFE += IFE1, l3dF += iFF, 
      //    l3dFEresi += IFE2
        TopOpeBRepDS_ListIteratorOfListOfInterference it3(begin(l3dF));
      found = FUN_find3dISEsameISF(I1,it3,end(l3dF));
      
      if (found) l3dFEresi.push_back(I1);
      else       lFE.push_back(I1);
      it1 = l3dFE.erase(it1);
...
//  Standard_Integer n3d =
  ...
  for (auto I : l3dF)
      mapftra.Add(I->Support());
  TopOpeBRepDS_ListOfInterference lII;
  TopOpeBRepDS_ListIteratorOfListOfInterference it1(begin(lI));
  while (it1 != end(lI)) {
    const Handle(TopOpeBRepDS_Interference)& I = *it1;
    Standard_Integer ITRASHA = I->Transition().Index();
    if (mapftra.Contains(ITRASHA)) {++it1;}
    else                           {lII.push_back(I);it1 = lI.erase(it1);}
  }
  ...
  while (it1 != end(lII)) {
	...
	complex2d = complex2d || FUN_ds_sdm(BDS,F1,F2);
      ...
      if (complex2d) {l2dI.push_back(I2); it2 = lII.erase(it2);}
      else           ++it2; 
    } // it2
    if (complex2d) {l2dI.push_back(I1); it1 = lII.erase(it1);}
    else           ++it1;
  } // it1
  lI.insert(end(lI), begin(lII), end(lII));
  n2d = l2dI.size();
  return n2d;
}
...
//                          E sdm ES1, ES2}     
{
  ...
  for (tki.Init(); tki.More(); tki.Next()) {
    ...
    ok = ok && (nloi > 1);
    if (!ok) {newLI.insert(end(newLI), begin(loi), end(loi)); continue;}
...
//                 LI.Extent(); //deb
	l1dI.push_back(I2);
      ...
      if (complex1d) {l1dI.push_back(I1); ++it1;}
      else           ++it1;
    } // it1
  } // tki
