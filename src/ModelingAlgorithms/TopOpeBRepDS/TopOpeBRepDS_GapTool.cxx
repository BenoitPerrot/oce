...
#include <ModelingAlgorithms/TopOpeBRepDS/TopOpeBRepDS_GapTool.hxx>
...
IMPLEMENT_STANDARD_TYPE_END(TopOpeBRepDS_GapTool)
...
void TopOpeBRepDS_GapTool::Init(const Handle(TopOpeBRepDS_HDataStructure)& HDS) 
{
  ...
  for (i = 1; i <= NbCurves; i++) {
    for (auto I : myHDS->ChangeDS().ChangeCurveInterferences(i)) {
      if (I->GeometryType() == TopOpeBRepDS_POINT) 
	StoreGToI(myGToI,I);
    }
  }
						     ...
						     TopoDS_Shape&                            E) const
{
  if (I->GeometryType() == TopOpeBRepDS_POINT) { 
    ...
    for (const Handle(TopOpeBRepDS_Interference)& II : myGToI(I->Geometry())) {
...
{
  if (IP != I->Geometry()) {
    for (Handle(TopOpeBRepDS_Interference) II : myGToI(I->Geometry())) {
      II->Geometry(IP);  
      StoreGToI(myGToI,II);
    }
  }
