...
void PLib::CoefficientsPoles (const Standard_Integer      dim,
			      ...
			      TColStd_Array1OfReal*       Weights)
{
  Standard_Boolean rat = WCoefs != nullptr;
...
{
  Standard_Boolean rat = WCoeffs != nullptr;
  ...
  for (Standard_Integer icol = lc; icol <= uc; icol++) {
    ...
    for ( irow = lr; irow <= ur; irow++) {
      ...
      if (rat) Temw (irow) = (*WCoeffs) (irow, icol); // FIXME: test on rat unclear
    }
    if (rat) PLib::Trimming (U1, U2, Temp, &Temw); // FIXME: unclear
    else PLib::Trimming (U1, U2, Temp, nullptr);

    for (irow = lr; irow <= ur; irow++) {
      Coeffs  (irow, icol) = Temp (irow);
      if (rat) (*WCoeffs) (irow, icol) = Temw (irow); // FIXME: unclear
    }
  }
}
		     ...
		     TColStd_Array2OfReal* WCoeffs)
{
  ...
  for (Standard_Integer irow = lr; irow <= ur; irow++) {
    ...
    for ( icol = lc; icol <= uc; icol++) {
      ...
      if (rat) Temw (icol) = (*WCoeffs) (irow, icol); // FIXME: test on rat is unclear
    }
    if (rat) PLib::Trimming (V1, V2, Temp, &Temw);
    else PLib::Trimming (V1, V2, Temp, nullptr);
