...
void stepset_debug (int debug_flag  );

YY_EXTRA_TYPE stepget_extra (void );

void stepset_extra (YY_EXTRA_TYPE user_defined  );
 ...
 * we now use fwrite().
 */
#define ECHO do { if (fwrite( steptext, stepleng, 1, stepout )) {} } while (0)
#endif
...
YY_DECL
{
	...
	if ( !(yy_init) )
        ...
        if ( ! (yy_state_buf) )
            ...
            if ( ! (yy_state_buf) )
                YY_FATAL_ERROR( "out of dynamic memory in steplex()" );
...
"input buffer overflow, can't enlarge buffer because scanner uses REJECT" );
	...
	if ((yy_size_t) ((yy_n_chars) + number_to_move) > YY_CURRENT_BUFFER_LVALUE->yy_buf_size) {
		...
		YY_CURRENT_BUFFER_LVALUE->yy_ch_buf = (char *) steprealloc((void *) YY_CURRENT_BUFFER_LVALUE->yy_ch_buf,new_size  );
...
static void stepensure_buffer_stack (void)
{
	...
	if (!(yy_buffer_stack)) {
		...
		if ( ! (yy_buffer_stack) )
			YY_FATAL_ERROR( "out of dynamic memory in stepensure_buffer_stack()" );
