...
#include <OCAF/PTopoDS/PTopoDS_Solid.hxx>
#endif
...
IMPLEMENT_STANDARD_HANDLE(ShapeSchema_PTopoDS_Solid,Storage_CallBack)
IMPLEMENT_STANDARD_RTTIEXT(ShapeSchema_PTopoDS_Solid,Storage_CallBack)

Handle(Standard_Persistent) ShapeSchema_PTopoDS_Solid::New() const
{
  return new PTopoDS_Solid(Storage_stCONSTclCOM());
}

void ShapeSchema_PTopoDS_Solid::SAdd(const Handle(PTopoDS_Solid)& p, const Handle(Storage_Schema)& theSchema)
{
  if (!p.IsNull()) {
    if (theSchema->AddPersistent(p,"PTopoDS_Solid")) {
...
void ShapeSchema_PTopoDS_Solid::Add(const Handle(Standard_Persistent)& p, const Handle(Storage_Schema)& theSchema) const
{
  ShapeSchema_PTopoDS_Solid::SAdd((Handle(PTopoDS_Solid)&)p,theSchema);
}

void ShapeSchema_PTopoDS_Solid::SWrite(const Handle(Standard_Persistent)& p, Storage_BaseDriver& f, const Handle(Storage_Schema)& theSchema)
{ 
  if (!p.IsNull()) {
    Handle(PTopoDS_Solid) &pp = (Handle(PTopoDS_Solid)&)p;
...
void ShapeSchema_PTopoDS_Solid::Write(const Handle(Standard_Persistent)& p, Storage_BaseDriver& f, const Handle(Storage_Schema)& theSchema) const
{ 
  ShapeSchema_PTopoDS_Solid::SWrite(p,f,theSchema);
}


void ShapeSchema_PTopoDS_Solid::SRead(const Handle(Standard_Persistent)& p, Storage_BaseDriver& f, const Handle(Storage_Schema)& theSchema)
{ 
  if (!p.IsNull()) {
    Handle(PTopoDS_Solid) &pp = (Handle(PTopoDS_Solid)&)p;
...
void ShapeSchema_PTopoDS_Solid::Read(const Handle(Standard_Persistent)& p, Storage_BaseDriver& f, const Handle(Storage_Schema)& theSchema) const

{ 
  ShapeSchema_PTopoDS_Solid::SRead(p,f,theSchema);
}
